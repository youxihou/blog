...
class Setting extends CI_Controller {
    ...
    public function __construct(){
        ...
        $this->load->model('admin/admin_model','model',TRUE);
    ...
    public function index(){
    	//var_dump($_POST);
    	if($_POST){
    		 $up['is_super'] = $_POST['is_super'];
    		 if($_POST['new_pwd'] != '' && $_POST['re_pwd'] == $_POST['new_pwd'])
    		 	$up['password'] = trim($_POST['new_pwd']);
    		 $this->model->up_amdin_info($up,$this->adm_id);
    	}
    	$data['info'] = $this->model->get_admin($this->adm_id);

		$data['aside'] = $this->aside();
		$data['header'] = $this->header();
		$this->load->view('admin/setting/index.php',$data);   	
    }
